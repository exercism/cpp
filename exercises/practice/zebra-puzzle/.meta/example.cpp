...
bool check_constraints_cigarette_drink_pet() 
{
    for (size_t i = 0; i < houses.size(); i++) 
    {
        ...
        if ((i > 0 && house.pet == Fox && houses[i - 1].cigarette != Chesterfields) &&
            (i < 4 && house.pet == Fox && houses[i + 1].cigarette != Chesterfields))
            return false;

        if (i == 0 && house.pet == Fox && houses[i + 1].cigarette != Chesterfields)
            return false;

        if (i == 4 && house.pet == Fox && houses[i - 1].cigarette != Chesterfields)
            return false;

        // Constraint 12: Kools are smoked in a house next to the house where
        // the horse is kept.
        if ((i > 0 && house.pet == Horse && houses[i - 1].cigarette != Kools) &&
            (i < 4 && house.pet == Horse && houses[i + 1].cigarette != Kools))
            return false;

        if (i == 0 && house.pet == Horse && houses[i + 1].cigarette != Kools)
            return false;

        if (i == 4 && house.pet == Horse && houses[i - 1].cigarette != Kools)
            return false;
    }
    return true;
}
...
bool check_constraints_cigarette_drink_pet_nationality_color() 
{
    for (size_t i = 0; i < houses.size(); i++) 
    {
        ...
        // ivory house.
        if (i > 0 && house.color == Green && houses[i - 1].color != Ivory)
            return false;
        ...
        if ((i > 0 && house.nationality == Norwegian && houses[i - 1].color != Blue) &&
        ...
        if (i == 4 && house.nationality == Norwegian && houses[i - 1].color != Blue)
            return false;
    }
    return true;
}
...
void solve_with_permutation() 
{
    ...
    vector<Nationality> nationalities = {Englishman, Spaniard, Ukrainian, Norwegian, Japanese};
    vector<Pet> pets = {Dog, Snails, Fox, Horse, Zebra};
    vector<Drink> drinks = {Tea, Coffee, Milk, OrangeJuice, Water};
    vector<Cigarette> cigarettes = {OldGold, Kools, Chesterfields, LuckyStrike, Parliaments};

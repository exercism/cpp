...
TEST_CASE("checking_balance_of_not_opened_account_throws_error") {
    Bankaccount account{};
    REQUIRE_THROWS_AS(account.balance(), std::logic_error);
}
...
TEST_CASE("deposit_into_closed_account_throws_error") {
    ...
    REQUIRE_THROWS_AS(account.deposit(50), std::logic_error);
}

TEST_CASE("withdraw_into_closed_account_throws_error") {
    Bankaccount account{};
    account.open();
    account.close();

    REQUIRE_THROWS_AS(account.withdraw(50), std::logic_error);
}

TEST_CASE("close_an_not_opened_account_throws_error") {
    Bankaccount account;

    REQUIRE_THROWS_AS(account.close(), std::logic_error);
}
...
TEST_CASE("open_an_already_opened_account_throws_error") {
    ...
    REQUIRE_THROWS_AS(account.open(), std::logic_error);
}
...
TEST_CASE("cannot_withdraw_more_than_deposited") {
    ...
    REQUIRE_THROWS_AS(account.withdraw(150), std::logic_error);
}

TEST_CASE("deposit_negativ_amount_throws_logic_error") {
    Bankaccount account;
    account.open();

    REQUIRE_THROWS_AS(account.deposit(-100), std::logic_error);
}

TEST_CASE("withdraw_negativ_amount_throws_logic_error") {
    Bankaccount account;
    account.open();

    REQUIRE_THROWS_AS(account.withdraw(-100), std::logic_error);
}

...
namespace book_store {
    ...
    static const int PRICE_AFTER_DISCOUNT[6] = {
        0,
        800,
        1520,
        2160,
    ...
    static int dfs(const std::array<int,5>& state, std::map<std::array<int,5>,int>& memo) {
        auto it = memo.find(state);
        if (it != memo.end()) return it->second;

        bool empty = true;
        for (int c : state) {
            if (c > 0) { empty = false; break; }
        }
